...
<script type="text/javascript" src="js/graphics.topdown.js"></script>
...
<textarea id="debug" style="font-weight:bold;width:600px;height:36px;display:block;position:absolute;top:0px;left:140px;border:none;background-color:transparent;color:#FFFFFF;"></textarea>
		...
		getStaticSpritesForPoint:function(x,y,requestSort) {
			...
			for (var i = 0; i < spriteSource.length + showGrass; i++) {
				var size = (Math.min(12,(
					getRandPoint((x/4)+rand[i*33]*20,(y/4)+rand[i*65]*20)*2-1
				)* 12)|0)*10;
...
function debug(txt,noClear,noCR) {
	document.getElementById('debug').value = "Lamenting Globule Rendering Engine Test\n"+(noClear?document.getElementById('debug').value+(noCR?"":"\n"):"")+txt;
};
